               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    int rowsPerThread = 0;
    int workSizeLastThread = dataSizeX*dataSizeY - (workSize*(numThreads-1)) + (kernelSizeX* dataSizeX);
    ...
    int *pointerToStartOut; //= auxOptr;
    int *auxInptr; //= inPtr + (myStart*dataSizeX);
    int *auxInptr2;// = auxInptr;
    int *pointerToStartInt;// = auxInptr;
    ...
    {   
        ...
        for(iLoop = 1; iLoop < numThreads; iLoop++)
        {
            ...
            threadRowStart = (dataSizeY/numThreads)*(iLoop); 
            ...
            if(iLoop == numThreads-1){
               workSize = workSizeLastThread;//dataSizeX*dataSizeY - (workSize*(numThreads-1)) + (kernelSizeY* dataSizeX);                
            }else{
                int inPtrRows = (dataSizeY/numThreads)*dataSizeX ;
                int kernelRows = (kernelSizeX*dataSizeX);
                workSize =  inPtrRows + kernelRows; // 1 row is X colums.
            }     
    //        printf("1- SEND TO: worksize: %d\t idThread: %d\n", workSize, iLoop);
            MPI_Send(pointerToStartInt, workSize, MPI_INT, iLoop, 0, MPI_COMM_WORLD); // tag 0 is out file        
        }        
        // variable rank 0
        auxInptr = inPtr;
        auxOptr = outPtr;
        workSize = (dataSizeY/numThreads)*dataSizeX + (kernelSizeX*dataSizeX);       
    }
    else
    {      
        /// variables childs
        auxInptr = (int*)malloc(sizeof(int)*workSize);
        auxOptr = (int*)malloc(sizeof(int)*workSize);
//printf("2- RECV FROM parent: worksize: %d\t idThread: %d\n", workSize, idThread);
        MPI_Recv(auxInptr, workSize, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);    
    }
    
    auxInptr2 = auxInptr;
    pointerToStartOut = auxOptr;
    rowsPerThread = workSize/dataSizeX;
    //printf("----------------rank %i: work:%i\n ",idThread,workSize);
    // start convolution
    //----------------------------------------------------------------------------------------
    
    for(i= 0; i < rowsPerThread; ++i)                   // number of rows
        {
            // compute the range of convolution, the current row of kernel should be between these
            rowMax = i  + kCenterY;
            rowMin = i - rowsPerThread + kCenterY;
    ...
    {        
        ...
        for(iLoop = 1; iLoop < numThreads; iLoop++)
        {
            ...
            if(iLoop == numThreads-1){
                workSize = workSizeLastThread;//dataSizeX*dataSizeY - (workSize*(numThreads-1)) + (kernelSizeX* dataSizeX);         
            }else{
                int inPtrRows = (dataSizeY/numThreads)*dataSizeX ;
                int kernelRows = (kernelSizeX*dataSizeX);
                workSize =  inPtrRows + kernelRows; // 1 row is X colums.
            }          
  //          printf("3- RECV parent: worksize: %d\t idThread: %d\n", workSize, iLoop);
            //MPI_Send(pointerToStartInt, workSize, MPI_INT, iLoop, 0, MPI_COMM_WORLD); // tag 0 is out file        
            int *resultOut;
            resultOut = (int*)malloc(sizeof(int)*(workSize+(dataSizeX*(kernelSizeY))));
            //printf("RECV2 rank %i: work:%i\n ",iLoop,workSize);
            MPI_Recv(resultOut, workSize, MPI_INT, iLoop, 0, MPI_COMM_WORLD, &status);
            //printf("RECV2 rank %i: work:%i\n ",iLoop,workSize);
//             if(idThread == numThreads -1)
             auxOptr = pointerToStartOut;
//             else 
//                auxOptr = pointerToStartOut + dataSizeX;
...
//               printf("%i\n",w);
    ...
    {      
        // work size is defined at the begining
        //printf("SEND2 rank %i: work:%i\n ",idThread,workSize);
//        printf("4- SEND to parent: worksize: %d\t idThread: %d\n", workSize, idThread);
